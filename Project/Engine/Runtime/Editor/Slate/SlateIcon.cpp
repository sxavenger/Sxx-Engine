...
Slate::IconGlyph::Data Slate::IconGlyph::Encode(Icon icon) noexcept {
	...
	if (codepoint < 0x80) {
		data[0] = static_cast<char8_t>(codepoint);

	} else if (codepoint < 0x800) {
		data[0] = static_cast<char8_t>(0xC0 | (codepoint >> 6));
		data[1] = IconGlyph::Continuation(codepoint);

	} else if (codepoint < 0x10000) {
		data[0] = static_cast<char8_t>(0xE0 | (codepoint >> 12));
		data[1] = IconGlyph::Continuation(codepoint >> 6);
		data[2] = IconGlyph::Continuation(codepoint);

	} else {
		data[0] = static_cast<char8_t>(0xF0 | (codepoint >> 18));
...
char8_t Slate::IconGlyph::Continuation(uint32_t value) noexcept {
	return static_cast<char8_t>(0x80 | (value & 0x3F));
}
