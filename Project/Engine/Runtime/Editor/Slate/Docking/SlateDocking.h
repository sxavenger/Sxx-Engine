...
namespace Slate {
	...
	////////////////////////////////////////////////////////////////////////////////////////////
	// docking tree traversal rule
	////////////////////////////////////////////////////////////////////////////////////////////
	//! @note 【重要】ここの走査関数は Splitter と DockTabStack だけを辿る. それ以外の型は素通りする.
	//!       ApplyDockingHost / ResetTabBarInsets / IsPointOnAnyTab / FindTopRightTabStack /
	//!       FindFirstTabStack / PruneEmptyNodes / RemovePanelFromTree が該当し,
	//!       CollectAllPanels は root が IDockNode でなければ何も返さない.
	//!       つまり タイトルバーを載せるために VerticalBox や Overlay を root 直下に挟むと,
	//!       1段目で打ち切られて host が配られない(空ノードの詰めもパネルの回収も全滅する).
	//! @note したがって ドッキングのツリー(dockRoot)と表示用のツリー(root)は必ず分けて持つこと.
	//!       dockRoot は Splitter と DockTabStack のみで構成し, chrome はそれを包む形にする.
	//!       dockRoot を差し替えたら chrome を巻き直す.
	//! @note 例外は FindDockTargetAt のみ. あれは OnArrangeChildren で降りるので任意の型を通過するが,
	//!       渡すジオメトリは dockRoot が実際に置かれている矩形にすること
	//!       (client全体を渡すと TitleBar の分だけマーカーの描画位置と当たり判定がずれる).
