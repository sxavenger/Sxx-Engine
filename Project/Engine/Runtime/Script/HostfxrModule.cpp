...
void HostfxrModule::Settings::Log(const Settings& settings) {
	STREAM_LOG_DEBUG("Script::HostfxrModule::Settings | filepath: {}", settings.filepath.generic_string());
}
...
void HostfxrModule::Load(const Configuration& config) {
	...
	if (settings_.filepath.empty()) {
		STREAM_LOG_INFO("Script::HostfxrModule | hostfxr.dll filepath is not set.");
		return; //!< hostfxr.dllのパスが設定されていない場合はロードしない.
	}

	//!< moduleの読み込み
	module_ = LoadLibraryW(settings_.filepath.generic_wstring().c_str());
	STREAM_ASSERT(module_ != nullptr, "failed to load hostfxr library. filepath: {}", settings_.filepath.generic_string());
	...
	STREAM_LOG_INFO("Script::HostfxrModule | loaded successfully.");
}
...
void HostfxrModule::Unload() {
	...
	STREAM_LOG_INFO("Script::HostfxrModule | unloaded successfully.");
}
...
hostfxr_handle HostfxrModule::CreateHandle(const std::filesystem::path& runtimeConfigPath) const {
	...
	STREAM_ASSERT(rc == 0 && handle != nullptr, "failed to initialize hostfxr handle.");
